...
//
// Ported into the Daggerfall Unity iOS touch port, under the MIT License declared above.
// Hazelnut also confirmed directly to the port author that this use is welcome.
//
// Changes made for the port, all mechanical - the road generation itself is untouched:
//   * Path data loads from Resources instead of ModManager.TryGetAsset(), because built-in
//     code has no mod to fetch assets from. iOS cannot load mod code at all, so this had to
//     be compiled in rather than shipped as a .dfmod.
//   * The in-game path editor is removed. It is a desktop authoring tool, and its input and
//     window handling do not exist on a touchscreen; the shipped path data is read-only here.
//   * No ModSettings. Options come from the port's own settings.
...
namespace BasicRoads
{
    ...
    public class BasicRoadsTexturing : DefaultTerrainTexturing
    {
        ...
        // Resources rather than a mod asset: this is compiled into the app, so there is no mod
        // to ask. The filenames keep their .bytes extension because Unity maps that to
        // TextAsset; Resources.Load wants the path without it.
        public const string ResourceFolder = "BasicRoads/";

        private static byte[] ReadPathData(string filename)
        {
            byte[] pathData = null;

            string resourceName = ResourceFolder +
        ...
        public JobHandle SchedulePaintRoadsJob(ref MapPixelData mapData, ref NativeArray<byte> tileData, JobHandle dependencies)
        {
            ...
            if (editing == "path")
            {
                roadDataPt = pathsData[roads][pathsIndex];
            ...
            if (editing == "water")
            {
                riverDataPt = pathsData[rivers][pathsIndex];
                riverCorners = (byte)(InRange(pathsIndex) ? (pathsData[rivers][pathsIndex + 1] & 0x5) | (pathsData[rivers][pathsIndex - 1] & 0x50) : 0);
                streamDataPt = pathsData[streams][pathsIndex];
                streamCorners = (byte)(InRange(pathsIndex) ? (pathsData[streams][pathsIndex + 1] & 0x5) | (pathsData[streams][pathsIndex - 1] & 0x50) : 0);
            }
