...
// File Scripts/Helpers/BufferIO.cs, copied unchanged for iOS except lines marked MOBILE.
...
namespace Monobelisk
{
    public static class BufferIO
    {
        ...
        /// 
        // MOBILE: (g) buffers taken by ref so that Dispose() nulling the five ComputeBuffer fields is
        // MOBILE: visible to the TerrainComputer that owns them - otherwise the caller's copy still held
        // MOBILE: released buffers and the sampler's finally would release them a second time.
        public static void ProcessBufferValuesAndDispose(ref HeightmapBufferCollection buffers, ref MapPixelData mapData)
        {
            ...
            InterestingTerrains.tileDataCache.Add(pos, tileData);   // MOBILE: (e) now an indexer assignment inside TileDataCache
