...
**Files:** `Assets/Shaders/DistantTerrain/DistantTerrainTilemap.shader`, `FarTerrainCommon.cginc` (verbatim upstream copies + provenance comment), `Assets/Editor/MobileShaderSpike.cs` (kept, reusable), `Assets/Editor/MobileModPackTextureRules.cs`, `MobileModBuilder.cs`, `MobileBuildSetup.cs`, `MobileSelfTest.cs`

**Interfaces:** Produces the spike record `.superpowers/sdd/2026-09-09-distant-terrain-port/spike-metal-compile.md` (compiled yes/no for Metal iOS, sampler count reported, every warning/error verbatim); `Rule.RawData` extended to folder `DistantTerrainWoD`, mips off, and a per-file single-channel hook `public static bool SingleChannel(string assetPath)` (true only for this mod's `daggerfall_deriv_map.png`) that makes the iOS override `TextureImporterFormat.R8` instead of `RGBA32`. **REVISED (Task 1 review): no `MaxSize` hook, no 8192.** `maxTextureSize` stays 2048 everywhere, so the 5000x2500 map imports at 2048x1024 R8 = 2 MB GPU + 2 MB readable copy, against ~100 MB for a full-size RGBA32 one. Safe because `DistantTerrain.ApplyDerivativeHeightmap` scales the image onto the 1000x500 world grid by the texture's OWN `width`/`height` and takes the darkest pixel per cell block - it hard-codes no resolution. Measured against a full-resolution carve, a 2048x1024 map loses 0.3% of water cells and gains 0.2% (a 1250x625 one: 1.0% / 0.4%). Both path rules are keyed on mod folder + file name, never the bare file name: `WorldOfDaggerfallTerrain` ships a different `daggerfall_deriv_map.png` under the LinearData rule.
...
- [ ] Step 2: RED/GREEN for the importer rule checks (deriv map -> RawData, `SingleChannel` true for it and false for the Biomes colour key and for the Terrain mod's same-named map, NoMips true for the deriv map, an R8 -> `GetPixels32().r` round-trip, imported size still oversamples the 1000x500 grid; Biomes unchanged); one-asset reimport proof on `daggerfall_deriv_map.png.meta` (`isReadable: 1`, iOS `textureFormat: 63` = R8, `maxTextureSize: 2048`, `overridden: 1`, `enableMipMap: 0`) plus the imported size logged by `ReimportPacks`.
- [ ] Step 3: commit `Distant Terrain: original shader copied for the Metal spike; deriv map imports raw and single-channel` (record the spike verdict in the report and the ledger).
